...
MetroPlannerWindow::MetroPlannerWindow(QWidget *parent) : QMainWindow(parent)
{
    ...
    chatHistory->setMinimumHeight(200);

    chatInput = new QLineEdit;
    chatInput->setPlaceholderText("Ask the assistant (e.g. 'How much is the fare?')");

    sendChatBtn = new QPushButton("Send");
    sendChatBtn->setFixedWidth(80);

    auto *chatInputLayout = new QHBoxLayout;
    chatInputLayout->addWidget(chatInput);
    chatInputLayout->addWidget(sendChatBtn);

    controlsLayout->addWidget(new QLabel("Chat Assistant"));
    controlsLayout->addWidget(chatHistory);
    controlsLayout->addLayout(chatInputLayout);

    settingsBtn = new QPushButton("Settings");
    settingsBtn->setToolTip("Open Chatbot settings (API key and enable remote)");
    controlsLayout->addWidget(settingsBtn);

    connect(settingsBtn, &QPushButton::clicked, this, &MetroPlannerWindow::openSettings);

    connect(sendChatBtn, &QPushButton::clicked, this, &MetroPlannerWindow::sendChat);
    connect(chatInput, &QLineEdit::returnPressed, this, &MetroPlannerWindow::sendChat);
    connect(chatbot, &Chatbot::responseReady, this, &MetroPlannerWindow::onChatResponse);
...
void MetroPlannerWindow::sendChat()
{
    const QString prompt = chatInput->text().trimmed();
...
void MetroPlannerWindow::handleRouteRequest(const QString &fromRaw, const QString &toRaw)
{
    ...
    QString bestFrom = findBestMatch(from);
    QString bestTo = findBestMatch(to);

    if (bestFrom.isEmpty() || bestTo.isEmpty())
    {
        chatHistory->append(QString("<b>Assistant:</b> I couldn't find matching stations for '%1' or '%2'. Please try exact station names.").arg(fromRaw, toRaw));
        return;
    }
